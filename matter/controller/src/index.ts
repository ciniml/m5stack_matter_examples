...
async function readPng(path: string): Promise<Buffer> {
    return new Promise((resolve, reject) => {
        fs.createReadStream(path).pipe(new PNG())
            .on("parsed", function(buffer) { resolve(buffer); })
    ...
    async start() {
        ...
        if (pairingCode !== undefined) {
            ...
            longDiscriminator =
                ...
                (await controllerStorage.get("longDiscriminator", 3841));
            if (longDiscriminator > 4095) throw new Error("Discriminator value must be less than 4096");
            setupPin = environment.vars.number("pin") ?? (await controllerStorage.get("pin", 20202020));
        }
        ...
        try {
            ...
            if (devices[1] && devices[1].number === 2) {
                ...
                const boolenState = devices[1].getClusterClient(BooleanStateCluster);
                if (boolenState !== undefined) {
                    let stateValue = await boolenState.getStateValueAttribute();
                    console.log("initial booleanState StateValue", stateValue);

                    boolenState.addStateValueAttributeListener(value => {
                        console.log("subscription stateValue", value);
                        stateValue = value;
                        try {
                            fs.writeFileSync("/dev/fb1", value ? pushedImage : releasedImage);
                        } catch (error) {
                        }
                    });
                }
